...
import in.tap.we.poli.analytic.jobs.transformers.VendorsTransformationJob
  ...
  override def job(step: String, inArgs: InArgs, outArgs: OutArgs)(implicit spark: SparkSession): CompositeJob = {
    step match {
      ...
      case "vendors-transformer" =>
        new VendorsTransformationJob(inArgs.asInstanceOf[OneInArgs], outArgs.asInstanceOf[OneOutArgs])
      case "vendors-merger" =>
